...
//numoftriangles - length of trianglesV
	...
	int moveActionIndex, int moveToTableActionIndex, int*** plan, int* planlength) 
{
	...
	*planlength = 0;
	...
	Graph task_planner(blocksV, numofblocks, trianglesV, numoftriangles, TableIndex, onV_start,  onV_start_length,  clearV_start, numofclear_start, 
		onV_goal,  onV_goal_length, clearV_goal, numofclear_goal);
	// int num = task_planner.Get_Heuristic(&task_planner.Vertices.back());
	// mexPrintf("Heuristic value is %d\n",num);
	...
	list<Node*> Final_Plan;
	Node* Graph_Goal;

	// Dijkstra to find the heuristic values
	while (!task_planner.open_set.empty()){
		Cur_Node = task_planner.open_set.top();
		// if(task_planner.is_Node_in_Graph(Cur_Node)){
		// 	continue;
		// }
		//task_planner.print_Node_Info(Cur_Node);
		if (task_planner.is_goal_in_node(Cur_Node, &task_planner.GoalNode)){
			Graph_Goal = Cur_Node;
			mexPrintf("Expanded States: %d\n",task_planner.Vertices.size());
			break;
		}
	...
	while(Graph_Goal->parent != NULL){
		Final_Plan.push_front(Graph_Goal);
		Graph_Goal = Graph_Goal->parent;
	}
	//Final_Plan.push_front(Graph_Goal);

	// mexPrintf("Plan Size is: %d \n", Final_Plan.size()-1);

	int numofsteps = Final_Plan.size();    
	*plan = (int**) malloc(numofsteps*sizeof(int*));
	int i;

	list<Node*>::iterator it =  Final_Plan.begin();
	for (i = 0; i < numofsteps; i++){
		(*plan)[i] = (int*) malloc(4*sizeof(int)); 

		(*plan)[i][0] = (*it)->action_index;
		(*plan)[i][1] =  (*it)->action_blocks[0];
		(*plan)[i][2] =  (*it)->action_blocks[1]; 
		(*plan)[i][3] = (*it)->action_blocks[2];
